...
This repository contains a prototype for **"Agnes"**, an AI-powered decision-support system designed to consolidate fragmented raw material sourcing for the Consumer Packaged Goods (CPG) industry.

- **Goal**: Automate compliance checks and substitution detection.
- **Workflow**:
  1. Ingest internal database data (e.g., BOMs, Suppliers).
  2. Use Large Language Models (LLMs) to determine if raw ingredients are genuinely substitutable.
  3. Verify compliance using external/mocked data (e.g., scraping supplier websites).
  4. Produce an explainable sourcing recommendation (an "evidence trail").

## Build, Test, and Development Commands

- **Environment Setup**: `./setup.sh` (Installs JupyterLab, pandas, sqlite3, and google-genai via pipx to avoid filesystem constraints).
- **Run Application**: `jupyter-lab`
- **Data Exploration**: Open `explore_data.ipynb` for initial DB testing.

## Coding Style & Naming Conventions

- Prefer **Jupyter Notebooks (`.ipynb`)** for fast prototyping, LLM prompt engineering, and step-by-step debugging.
- Isolate complex logic (database querying, web scraping) into modular Python files (e.g., `database_utils.py`, `llm_reasoning.py`) as the prototype matures.
- Heavy focus on building an **"evidence trail"** for AI outputs rather than polished UI. Use Streamlit for the final demo if time permits.

## Key Constraints & Strategy

- **Web Scraping**: Can be notoriously brittle. Hardcode or mock retrieval for a few target sites if needed for the 1.5-day sprint to avoid getting blocked.
- **Hallucination Control**: Extremely important for judging. Ensure the LLM justifies its recommendations with external sources.
- **Scope**: Do not build a massive, all-encompassing system. Narrow the domain to a specific ingredient category (e.g., "Cocoa Powder") and prove the AI works perfectly on that subset.
...
- and producing an **explainable sourcing proposal** that - balances supplier consolidation, - lead time, - and practical feasibility.
