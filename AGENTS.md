...
- `start.sh` — Production launcher with watchdog (run: `./start.sh`). Checks prerequisites (.env, deps, KB), tees output to `logs/agnes_ui.log`, auto-opens browser, and auto-restarts on crash. Graceful Ctrl+C with uptime/restart summary.
- `download_models.py` — One-shot script to cache ML models locally (~175 MB). Downloads `all-MiniLM-L6-v2` (embeddings) and `cross-encoder/ms-marco-MiniLM-L-6-v2` (reranker) to `models/` so Agnes starts fully offline. Run once after cloning or when models/ is missing.
